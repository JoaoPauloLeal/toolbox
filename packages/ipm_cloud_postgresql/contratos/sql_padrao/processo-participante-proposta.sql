...
from (
    select
        ...
        (select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'processo', qcp.clicodigo, qcp.minano, qcp.minnro))) as id_processo,
        (select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'processo-participante', qcp.clicodigo, qcp.minano, qcp.minnro, (regexp_replace(u.unicpfcnpj,'[/.-]|[ ]','','g'))))) as id_participante,
        (select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'processo-item', qcp.clicodigo, qcp.minano, qcp.minnro, '@', coalesce(i.lotcodigo, 0) , '@', i.cmiitem))) as id_item,
        (select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'processo-participante-proposta', qcp.clicodigo, qcp.minano, qcp.minnro, (regexp_replace(u.unicpfcnpj,'[/.-]|[ ]','','g')), qcp.cmiid))) as id_gerado

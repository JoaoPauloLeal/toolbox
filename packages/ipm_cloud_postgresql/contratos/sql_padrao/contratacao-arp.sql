...
from (
	select
		...
		coalesce((select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'ata-rp', a.clicodigo, a.arpano, a.arpnro, a.unicodigo))), 0) as id_ata,
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'processo', a.clicodigo, a.minano, a.minnro))) as id_processo,
		...
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'entidade', a.clicodigo))) as id_entidade,
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'contratacao-arp', a.clicodigo, a.arpano, a.arpnro, '@', a.arpsequencia))) as id_gerado

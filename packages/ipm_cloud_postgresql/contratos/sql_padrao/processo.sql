...
from (
	select
		...
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'parametro-exercicio', p.pcsano))) as id_parametro_exercicio,
		p.loccodigo as local_entrega,
		coalesce((select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'local-entrega', (select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat('305', 'entidade', p.clicodigo))), p.loccodigo))), 0) as id_local_entrega,
		...
		(case  -- Regra para garantir que RP seja sempre Compras e Serviços, caso contrário retornará erro na forma de contratação
			...
			else (select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305,'tipo-objeto', (case m.mintipoobjeto when 1 then 'Compras e Outros Serviços' when 2 then 'Obras e Serviços de Engenharia' when 3 then 'Concessoes e Permissões de Serviços Públicos' when 4 then 'Alienação de bens'  when 5 then 'Concessão e Permissão de Uso de Bem Público' when 6 then 'Aquisição de bens' when 7 then 'Contratação de Serviços' else 'Compras Outros Serviços' end))))
		end) as id_tipo_objeto,
	    concat(coalesce(m.mintipojulgamento, 1), '-', m.mintipocomparacao) as forma_julgamento,
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'forma-julgamento', 2016, coalesce(m.mintipojulgamento, 1), m.mintipocomparacao))) as id_forma_julgamento,
		e.edtpreventrmat as prazo_entrega,
	    coalesce((select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'prazo-entrega', (select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat('305', 'entidade', 2016))), upper(unaccent(left(coalesce(trim(e.edtpreventrmat),'Imediata'), 50)))))), 0) as id_prazo_entrega,
		e.edtcondpgto as forma_pagamento,
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'forma-pagamento', coalesce(upper(unaccent(trim(e.edtcondpgto))), 'Conforme Edital')))) as id_forma_pagamento,
		--3166 as id_forma_pagamento, -- 10 dias
		m.mintiporegexec as regime_execucao,
        coalesce((select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'regime-execucao', (case m.mintiporegexec when 1 then 'Empreitada por Preço Global' when 2 then 'Empreitada por Preço Unitário' when 3 then 'Empreitada por Preço Global Integral' when 4 then 'Tarefa' when 5 then 'Execução Direta' when 6 then 'Cessão de Direitos' when 7 then 'Serviços' when 8 then 'Alienação de Bens Móveis' when 9 then 'Alienação de Bens Imóveis'  when 10 then 'Cessão de Direitos' when 11 then 'Cessão de Direito Real de Uso - Bens Públicos' else 'Compras' end)))), 0) id_regime_execucao,
		...
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat('305', 'responsavel', (select (regexp_replace(u.unicpfcnpj,'[/.-]|[ ]','','g')) from wun.tbunico u where u.unicodigo = (select i.unicodigo from wco.tbintegrante i where i.cmlcodigo = m.cmlcodigo and i.mbcatribuicao in(3,6) limit 1))))) as id_responsavel,
		m.cmlcodigo as cod_comissao,
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'comissao', m.cmlcodigo))) as id_comissao,
		p.modcodigo as cod_modalidade,
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'modalidade-licitacao', (select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'entidade', p.clicodigo))), p.modcodigo, (case when p.modcodigo = 6 then 1 else null end)))) as id_modalidade,
		...
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'processo', m.clicodigo, m.minano, m.minnro))) as id_gerado
	from wco.tbminuta m

...
from (
	select distinct
       ...
       'NENHUMA' as formaEspecial,
        (select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'forma-julgamento', clicodigo, mintipojulgamento, mintipocomparacao))) as id_gerado
	from wco.tbminuta

...
from (
	select
		*,
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'contratacao', aux.clicodigo, aux.ano_contrato, aux.identificador_superior))) as id_contratacao,
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'contratacao-item', aux.clicodigo, aux.ano_contrato, identificador_superior, '@', aux.cmiid))) as id_contratacao_item,
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'contratacao-aditivo', aux.clicodigo, aux.ano_contrato, nro_contrato))) as id_aditivo,
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'contratacao-aditivo-item', aux.clicodigo, aux.ano_contrato, nro_contrato, '@', aux.cmiid))) as id_gerado
	from (
		select
			c.clicodigo,

...
from (
	select
		...
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'parametro-exercicio', c.copano))) as id_exercicio,
		...
		c.copcondpgto,
		coalesce((select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'forma-pagamento', upper(unaccent(trim(c.copcondpgto)))))),
				 (select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'forma-pagamento', 'Conforme Edital')))
		) as id_forma_pagamento,
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'compra-direta', c.clicodigo, c.copano, c.copnro))) as id_gerado
	from wco.tbcompra c

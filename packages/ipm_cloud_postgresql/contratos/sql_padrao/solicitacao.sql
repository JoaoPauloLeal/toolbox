...
from (
	select
       (select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat('305', 'entidade', clicodigo))) as chave_dsk1,
       ...
       (select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'centro-custo', rqcano, replace((select cncclassif from wun.tbcencus where wun.tbcencus.cnccodigo = wco.tbreqcomp.cnccodigo),'.','')))) as id_organograma,
       coalesce((select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'local-entrega', (select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat('305', 'entidade', 2016))), loccodigo))), 14054) as id_local_entrega,
       (select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat('305', 'entidade', clicodigo))) as id_entidade_gestora,
	   ...
	   (select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat('305', 'solicitacao', (select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat('305', 'entidade', clicodigo))), rqcano, rqcnro))) as id_gerado
	from wco.tbreqcomp

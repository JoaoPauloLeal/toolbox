...
from (
	select
		dot.clicodigo,
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'entidade', dot.clicodigo))) as id_entidade,
		dot.loaano,
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'parametro-exercicio', dot.loaano))) as id_exercicio,
		dot.dotcodigo,
		...
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'despesa', (select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'entidade', dot.clicodigo))), dot.loaano, dot.dotcodigo))) as id_gerado

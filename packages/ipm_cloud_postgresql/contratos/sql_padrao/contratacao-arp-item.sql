...
from (
	select
		...
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'contratacao-arp', clicodigo_chave, aux.arpano, aux.arpnro, '@', aux.arpsequencia))) as id_contratacao,
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'material', im.prdcodigo))) as id_material,
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'material-especificacao', im.prdcodigo))) as id_especificacao,
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'material-duplicado-especificacao', aux.clicodigo_chave, aux.minano, aux.minnro, '@', aux.cmiid))) as id_material_duplicado_epec,
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'ata-rp', clicodigo_chave, aux.arpano, aux.arpnro, aux.unicodigo))) as id_ata,
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'ata-rp-item', clicodigo_chave, aux.arpano, aux.arpnro, aux.unicodigo, aux.cmiid))) as id_item_ata,
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'processo-participante-proposta', clicodigo_chave, aux.minano, aux.minnro, (regexp_replace(u.unicpfcnpj,'[/.-]|[ ]','','g')), aux.cmiid))) as id_proposta,
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'contratacao-arp-item', clicodigo_chave, aux.arpano, aux.arpnro, '@', aux.arpsequencia, '@', aux.cmiid))) as id_gerado
	from (
		select

...
from (
	select
		...
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'responsavel', cpf_cnpj))) as id_responsavel,
		...
		array(
			select json_build_object(
				...
				'id_logradouro', (select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305,
																													'logradouro',
																													logradouro,
																													(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305,
																																																			   'municipio',
																																																			   municipio,
																																																			   (select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'estado', estado))))))))),
				'numero', numero,
				'complemento', complemento,
				'bairro', bairro,
				'id_bairro', (select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305,
																														'bairro',
																														bairro,
																														(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305,
																																																				   'municipio',
																																																				   ...
																																																				   (select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'estado', estado))))))))),
				'municipio', municipio,
				'id_municipio', (select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'municipio', municipio, (select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'estado', estado)))))),
				'estado', estado,
				'id_estado', (select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'estado', estado))))
			from (
				select
					e.unicodigo,
					e.cplcep as cep,
		...
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat('305', 'fornecedor', cpf_cnpj))) as id_gerado
	from (

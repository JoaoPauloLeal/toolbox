...
from (
	 select
        ...
        concat(coalesce(e.edtdataentrprop, m.mindata, p.pcsdataproc), ' ', coalesce(e.edthoraentrprop, '00:00:00')) as dh_andamento,	 	null as observacao,
	 	m.cmlcodigo,
	 	array(select distinct id_gerado from public.controle_migracao_registro_compras where sistema = 305 and tipo_registro = 'comissao-membros' and i_chave_dsk1 = (select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'comissao', m.cmlcodigo)))::varchar) as array_membros,
	 	(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat('305', 'fornecedor', (select (regexp_replace(u.unicpfcnpj,'[/.-]|[ ]','','g')) from wun.tbunico u where u.unicodigo = (select mi.unicodigo from wco.tbintegrante mi where mi.cmlcodigo = m.cmlcodigo and mi.mbcatribuicao in (3, 6) order by mbcdatainicio desc limit 1))))) id_presidente,
	 	null as participante,
	 	(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'processo', m.clicodigo, m.minano, m.minnro)))  as id_processo,
	 	(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'processo-forma-contratacao', m.clicodigo, m.minano, m.minnro))) as id_forma_contratacao,
	 	(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'processo-sessao', m.clicodigo, m.minano, m.pcsnro))) as id_gerado
	 from wco.tbminuta m

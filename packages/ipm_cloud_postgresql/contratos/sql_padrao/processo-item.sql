...
from (
	-- Processos 'por item'
	 (select
		...
		, false) as possui_duplicado,
	 	(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'processo', i.clicodigo, i.minano, i.minnro))) as id_processo,
	 	...
	 	coalesce((select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'material-duplicado-especificacao', i.clicodigo, i.minano, i.minnro, '@', i.cmiid))), 0) as id_material_duplicado_epec,
        (select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'processo-item', i.clicodigo, i.minano, i.minnro, '@', 0, '@', i.cmiitem))) as id_gerado
     from wco.tbitemin i
	...
	(select
		...
		, false) as possui_duplicado,
	 	...
	 	(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'material-especificacao', i.prdcodigo))) as id_material_especificacao,
	 	coalesce((select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'material-duplicado-especificacao', i.clicodigo, i.minano, i.minnro, '@', i.cmiid))), 0) as id_material_duplicado_epec,
        (select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'processo-item', i.clicodigo, i.minano, i.minnro, '@', i.lotcodigo, '@', i.cmiitem))) as id_gerado
     from wco.tbitemin i
	 where i.clicodigo = {{clicodigo}}
	 and i.minano = {{ano}}
	 and i.minnro in (81)

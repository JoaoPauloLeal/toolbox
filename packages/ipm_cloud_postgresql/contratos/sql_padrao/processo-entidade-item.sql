...
from (
	select
		...
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'processo', d.clicodigomin, ano_processo, nro_processo))) as id_processo,
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'processo-item', d.clicodigomin, ano_processo, nro_processo, '@', nro_lote, '@', numero_item_original))) as id_item,
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'processo-entidade', d.clicodigomin, ano_processo, nro_processo, d.clicodigoreq))) as id_entidade_participante,
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'processo-entidade-item', d.clicodigomin, ano_processo, nro_processo, '@', nro_lote, '@', numero_item_original, '@', d.clicodigoreq)))  as id_gerado
	from (
		select
			r.clicodigomin,

...
from (
	(select
		...
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'contratacao-sf-recebimento', i.clicodigo, i.copano, i.copnro, '@', i.nfisequencia))) as id_recebimento,
		(select id_gerado from public.controle_migracao_registro_compras where tipo_registro = 'contratacao-sf-item' and i_chave_dsk1 = i.clicodigo::varchar and i_chave_dsk5 = c.copano::varchar and i_chave_dsk6 = c.copnro::varchar and i_chave_dsk8 = (select ic.cmiid from wco.tbitemcompra ic where ic.clicodigo = c.clicodigo and ic.copano = c.copano and ic.copnro = c.copnro and ic.itcitem = i.itcitem)::varchar) as id_sf_item,
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'contratacao-sf-recebimento-item', i.clicodigo, i.copano, i.copnro, '@', i.nfisequencia, '@', i.itcitem))) as id_gerado

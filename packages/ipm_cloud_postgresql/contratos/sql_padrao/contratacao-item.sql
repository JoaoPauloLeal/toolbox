...
from (
	select distinct
		...
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'contratacao', c.clicodigo, c.ctrano, c.ctridentificador))) as id_contratacao,
		...
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'material-especificacao', i.prdcodigo))) as id_especificacao,
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'material-duplicado-especificacao', i.clicodigo, i.minano, i.minnro, '@', i.cmiid))) as id_material_duplicado_espec,
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'processo-ato-final', coalesce(c.clicodigomin, c.clicodigo), c.minano, c.minnro))) as id_ato_final,
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'processo-participante-proposta', coalesce(c.clicodigomin, c.clicodigo), q.minano, q.minnro, (regexp_replace(u.unicpfcnpj,'[/.-]|[ ]','','g')), q.cmiid))) as id_proposta,
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'contratacao-item', c.clicodigo, c.ctrano, c.ctridentificador, '@', q.cmiid))) as id_gerado

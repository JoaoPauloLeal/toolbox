...
from (
    select
        ...
        (case
            when p.modcodigo = 5 then (select i_chave_dsk2 from public.controle_migracao_registro_compras where sistema = '305' and tipo_registro = 'comissao-membros' and i_chave_dsk1 = (select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'comissao', m.cmlcodigo)))::varchar and i_chave_dsk3 in ('LEILOEIRO') limit 1)
            else (select i_chave_dsk2 from public.controle_migracao_registro_compras where sistema = '305' and tipo_registro = 'comissao-membros' and i_chave_dsk1 = (select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'comissao', m.cmlcodigo)))::varchar and i_chave_dsk3 in ('PRESIDENTE', 'PREGOEIRO') limit 1)
        end) as cpf_membro_comissao,
        (case
            when p.modcodigo = 5 then coalesce((select id_gerado from public.controle_migracao_registro_compras where sistema = '305' and tipo_registro = 'comissao-membros' and i_chave_dsk1 = (select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'comissao', m.cmlcodigo)))::varchar and i_chave_dsk3 in ('LEILOEIRO') limit 1), 0)
            else coalesce((select id_gerado from public.controle_migracao_registro_compras where sistema = '305' and tipo_registro = 'comissao-membros' and i_chave_dsk1 = (select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'comissao', m.cmlcodigo)))::varchar and i_chave_dsk3 in ('PRESIDENTE', 'PREGOEIRO') limit 1), 0)
        end) as id_membro_comissao,
        p.modcodigo as cod_modalidade,
        (select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'modalidade-licitacao', (select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'entidade', p.clicodigo))), p.modcodigo, m.mintipopregao))) as id_modalidade,
        ...
        (select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'processo', p.clicodigo, m.minano, m.minnro))) as id_processo,
        (select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'processo-forma-contratacao', p.clicodigo, m.minano, m.minnro))) as id_gerado
    from wco.tbminuta m

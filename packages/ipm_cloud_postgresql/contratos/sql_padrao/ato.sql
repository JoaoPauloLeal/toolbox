...
from (
	select distinct
	    ...
	    (select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat('305','natureza-texto-juridico', ato.tctcodigo))) as id_natureza_texto_juridico,
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat('305','tipo-ato', ato.tctcodigo))) as id_tipo_ato,

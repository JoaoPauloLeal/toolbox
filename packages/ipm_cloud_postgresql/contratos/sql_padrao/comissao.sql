...
from (
	select
		...
		c.txjcodigo as ato,
		coalesce((select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'tipo-ato', ato.tctcodigo))),0) as id_tipo_ato,
		coalesce((select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'ato', ato.txjnumero, ato.txjano, (select upper(tipo_ato.tctdescricao) from wlg.tbcategoriatexto tipo_ato where tipo_ato.tctcodigo = ato.tctcodigo and tipo_ato.tcttipo = ato.tcttipo)))),0) as id_ato,
		--79054 as id_ato,
		--2612 as id_tipo_ato,
		array(
			select json_build_object(
				...
				'id_responsavel', (select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'responsavel', (regexp_replace(un.unicpfcnpj,'[/.-]|[ ]','','g'))))),
		...
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'comissao', c.cmlcodigo))) as id_gerado

...
from (
	(select distinct
		...
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'contratacao', nf.clicodigo, (select ct.ctranosup from wco.tbcontrato ct where ct.clicodigo = c.clicodigoctr and ct.minano = t.minano and ct.minnro = t.minnro and ct.ctridentificador = c.ctridentificador), (select ct.ctridentsup from wco.tbcontrato ct where ct.clicodigo = c.clicodigoctr and ct.minano = t.minano and ct.minnro = t.minnro and ct.ctridentificador = c.ctridentificador)))) as id_contratacao,
		(select id_gerado from public.controle_migracao_registro_compras where tipo_registro = 'contratacao-sf' and i_chave_dsk1 = c.clicodigoctr::varchar and i_chave_dsk5 = c.copano::varchar and i_chave_dsk6 = c.copnro::varchar) as id_sf,
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'contratacao-sf-recebimento', nf.clicodigo, nf.copano, nf.copnro, '@', nf.nfisequencia))) as id_recebimento,
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'contratacao-sf-recebimento-comprovante', nf.clicodigo, nf.copano, nf.copnro, '@', nf.nfisequencia))) as id_gerado

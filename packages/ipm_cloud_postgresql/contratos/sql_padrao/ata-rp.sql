...
from (
	select
	    ...
	    'ANDAMENTO' AS situacao,
	 	(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'parametro-exercicio', rp.arpano))) as id_parametro_exercicio,
	 	...
	 	(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'processo-ato-final', rp.clicodigo, rp.minano, rp.minnro))) as id_ato_final,
	 	...
	 	(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'ata-rp', rp.clicodigo, rp.arpano, rp.arpnro, rp.unicodigo))) as id_gerado

...
from (
	select distinct
		...
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'processo', dl.clicodigo, dl.minano, dl.minnro))) as id_processo,
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'processo-participante', dl.clicodigo, dl.minano, dl.minnro, (regexp_replace(u.unicpfcnpj,'[/.-]|[ ]','','g'))))) as id_participante,
	    (select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'tipo-documento', (select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat('305', 'entidade', 2016))), dl.doccodigo))) as id_tipo_documento,
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'processo-documento', dl.clicodigo, dl.minano, dl.minnro, dl.doccodigo))) as id_doc_processo,
		(select id_gerado from public.controle_migracao_registro_compras where hash_chave_dsk = md5(concat(305, 'processo-participante-documento', dl.clicodigo, dl.minano, dl.minnro, (regexp_replace(u.unicpfcnpj,'[/.-]|[ ]','','g')), dl.doccodigo))) as id_gerado

select
	(select first id_gerado from bethadba.controle_migracao_registro where tipo_registro = 'tipo_situacao' and i_chave_dsk1 = {{id_entidade}} and i_chave_dsk3 = tab.situacao) as id_gerado_tipo,
	CASE
		when tab.data_situacao is null
			then tab.data_assinatura
		else tab.data_situacao
	end as data_situacao,
	(select first id_gerado from bethadba.controle_migracao_registro where tipo_registro = 'convenio_repassado_aditivo' and i_chave_dsk1 = {{id_entidade}}
	and i_chave_dsk2 like isnull(bethadba.dbf_retira_alfa_de_inteiros(tab.i_convenio),'')) as id_gerado_convenio_aditivo,
...
from
	...
	'convenio_repassado_aditivo_situacao',
	{{id_entidade}},
	id_gerado_convenio,
	id_gerado_tipo,
	tab.data_situacao

...
select
	DISTINCT 
	tab.lei_convenio as lei_convenio,
	min(tab.data_lei_convenio) as data_lei_convenio,
	tab.natureza as natureza,
	(select first descricao from sapo.naturezas_juridicas where nivel = natureza) as descricao_natureza,
	(select first id_gerado from bethadba.controle_migracao_registro where tipo_registro = 'natureza_texto_juridico' and i_chave_dsk1 = {{id_entidade}} and i_chave_dsk2 = descricao_natureza) as id_gerado_natureza,
	(select first id_gerado from bethadba.controle_migracao_registro where tipo_registro = 'tipo_ato' and i_chave_dsk1 = {{id_entidade}} and i_chave_dsk2 = 'Convênio') as id_gerado_tipo_ato
...
from
	...
	SELECT
		...
		(select first cgc from sapo.credores c where nome like sapo.convenios.orgao_concedente and cgc is not null) as cnpj_cpf_concedente,
 ...
 sapo.convenios.arquivo_anexo,
	...
	WHERE
		( sapo.convenios.i_entidades = {{id_entidade}} )
			...
			GROUP BY tab.lei_convenio, tab.natureza;
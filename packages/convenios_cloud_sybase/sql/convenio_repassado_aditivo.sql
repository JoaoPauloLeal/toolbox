select
	bethadba.dbf_retira_alfa_de_inteiros(tab.i_convenio) as numero_convenio,
	(select id_gerado from bethadba.controle_migracao_registro where tipo_registro = 'convenio_repassado' and i_chave_dsk1 = {{id_entidade}}
	and i_chave_dsk2 like isnull(tab.convenio_superior,'')) as id_gerado_convenio,
	(
	select
		first id_gerado
	from
	...
	from
		bethadba.controle_migracao_registro cmr
	where
		tipo_registro in ('tipo_aditivo')
		and i_chave_dsk1 = {{id_entidade}}
		and i_chave_dsk2 = 'Acréscimo') as tipo_aditivo,
	(select i_chave_dsk6 from bethadba.controle_migracao_registro where tipo_registro = 'convenio_repassado' and i_chave_dsk1 = {{id_entidade}}
	and i_chave_dsk2 like isnull(tab.convenio_superior,'')) as data_assinatura_convenio_superior,
	(select i_chave_dsk5 from bethadba.controle_migracao_registro where tipo_registro = 'convenio_repassado' and i_chave_dsk1 = {{id_entidade}}
	and i_chave_dsk2 like isnull(tab.convenio_superior,'')) as data_fim_convenio_superior,
	CASE
		when tab.data_assinatura < data_assinatura_convenio_superior
			then data_assinatura_convenio_superior
		else tab.data_assinatura
	end as data_assinatura_time,
	cast(data_assinatura_time as DATE) as data_assinatura,
	isnull((
	select
		first id_gerado
	from
	...
	LEFT('Aditivo oriundo de migração | ' + tab.objeto_convenio, 250)+'...' as justificativa
from
	(
	SELECT
	...
	WHERE
		...
		--and (sapo.convenios.i_convenio like '%-1967%'
		--or sapo.convenios.i_convenio like '%/1967%')
		ORDER BY
			sapo.convenios.i_entidades ASC,
			sapo.convenios.i_convenio ASC ) as tab
		where tab.convenio_superior is not null and
	bethadba.dbf_get_situacao_registro({{sistema}},
	'convenio_repassado_aditivo',
	{{id_entidade}},
	numero_convenio,
	justificativa,
	periodo_inicio,
	periodo_fim) in(5, 3)
	ORDER BY tab.i_convenio ASC